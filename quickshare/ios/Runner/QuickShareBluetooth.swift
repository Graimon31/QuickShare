...
private enum QuickShareBleControl {
  ...
  /// Builds the `AP:` write — see parseApOffer for what one means. The parts
  /// are percent-encoded exactly as Dart's Uri.encodeComponent does, so the
  /// separator can never be mistaken for part of a credential.
...
public final class QuickShareBluetoothPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
  ...
  public func handle(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
    ...
    // The receiver's half of the negotiation: the credentials of the network
    // it raised, written back over the control characteristic.
    case "sendApOffer":
      guard let args = call.arguments as? [String: Any],
            let ssid = args["ssid"] as? String,
            let passphrase = args["passphrase"] as? String,
            let peripheral = targetPeripheral,
            let control = remoteControl else {
      ...
      peripheral.writeValue(
        Data(QuickShareBleControl.apOffer(ssid, passphrase).utf8),
  ...
  private func startAdvertising(items: [QuickShareSenderItem],
    ...
    // The public half of the session code when there is one: a receiver who
    // was read the digits derives the same value and picks this device out of
    // several, without the code itself ever going over the air. Falling back
...
extension QuickShareBluetoothPlugin: CBPeripheralManagerDelegate {
  ...
  public func peripheralManager(_ peripheral: CBPeripheralManager, didReceiveWrite requests: [CBATTRequest]) {
    for request in requests {
      ...
      // A receiver that raised the network itself says where. What to do
      // with the credentials is the coordinator's call in Dart.
      if let offer = QuickShareBleControl.parseApOffer(command) {
        peripheral.respond(to: request, withResult: .success)
